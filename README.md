...
- 支持自定义标签映射，可按行填写精确、通配符或正则规则，例如 `GND=G`、`PA*=A$1`、`/^PB(\d+)$/=B$1`。
...
- 右侧是希望最终显示在丝印上的文字。
- 支持使用 `=`、`:`、`->`、`=>` 作为分隔符。
- 推荐正则规则使用 `=`、`->`、`=>` 作为分隔符，不要用 `:`；`re:` 本身包含前缀冒号，继续用 `:` 做分隔符会产生歧义。
- 支持精确匹配、`*` / `?` 通配符，以及 `/.../` 或 `re:` 开头的正则规则。
- 通配符和正则规则默认都按“整条解析后标签”匹配，不是局部子串替换。
- `/.../flags` 按 JavaScript 正则字面量解析；如果模式内部需要匹配字符 `/`，请写成 `\/`。
- `re:` 规则只写模式本体，不需要再包一层 `/.../`；例如 `re:(?:.*?)\/?P(\d)\.(\d)\/?(?:.*?)=P$1.$2`。
- 通配符和正则规则都支持在右侧使用 `$1`、`$2` 这样的捕获替换。
- 标签解析会尽量保留标签正文里的 `.` 和 `/`；像 `P0.1`、`P0/1`、`P1.1/T2EX` 这类名称不会再被自动截断成尾段数字。
